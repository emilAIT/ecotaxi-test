...
void ReportPage::setHeader()
{
    ...
    case Report::DriverCharges:
        ...
        ui->ReportButton->setText("ЗАРЯДКА ВОДИТЕЛЕЙ");
        break;
...
void ReportPage::setTable()
{
    ...
    case Report::DriverCharges:
        model->setHorizontalHeaderLabels({"Водитель", "Заряжено (KWH)"});
        for (const QVariant &chargeData : ReportOperations::getDriverChargeReport(this->id, this->fromDate, this->toDate))
        {
            QVariantList charge = chargeData.toList();
            QList<QStandardItem *> row;
            row.append(new QStandardItem(charge[0].toString())); // Водитель
            QStandardItem *kwhItem = new QStandardItem();
            kwhItem->setData(charge[1].toInt(), Qt::DisplayRole); // KWH
            row.append(kwhItem);
            model->appendRow(row);
        }
        break;
...
void ReportPage::setBottomTable()
{
    if (this->id > 0) {
        ...
        case Report::DriverCharges:
            if (true)
            {
                QVariantList report = Operations::selectAllDriveCharges(this->fromDate, this->toDate); // Новый метод из Operations
                model->setHorizontalHeaderLabels({
                    "Имя водителя",
                    "Сколько зарядился (KWH)"
                });

                for (int i = 0; i < report.size(); i += 2)
                {
                    QList<QStandardItem *> row;
                    row.append(new QStandardItem(report[i].toString()));       // Имя водителя
                    row.append(new QStandardItem(report[i + 1].toString()));   // Сколько зарядился (KWH)
                    model->appendRow(row);
                }
            }
            break;
...
void ReportPage::setTableSizes()
{
    ...
    case Report::DriverCharges:
        ui->tableView->setColumnWidth(0, 150); // ID водителя
        ui->tableView->setColumnWidth(1, 200); // Имя водителя
        ui->tableView->setColumnWidth(2, 180); // Дата зарядки
        ui->tableView->setColumnWidth(3, 180); // Количество KWH
        ui->tableView->setColumnWidth(4, 180); // Стоимость
        break;
...
void ReportPage::on_ToPDFButton_clicked()
{
    ...
    case Report::DriverCharges:
        title = "Отчет по зарядке водителя " + Operations::getDriver(this->id).getName();
        start = 0;
        break;
