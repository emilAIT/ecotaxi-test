...
void GeneralReport::setReport(Report mode, QDate from, QDate to)
{
    ...
    if (this->mode == Report::Cars) {
        ui->PDFbyDate->setVisible(true);
    } else {
        ui->PDFbyDate->setVisible(false);
    }
...
void GeneralReport::setHeader()
{
    ...
    case Report::ChargesByDrivers:
        ui->Header->setText("ПО ЗАРЯДКАМ ВОДИТЕЛEЙ");
...
void GeneralReport::setTable()
{
    ...
    case Report::FinesByDrivers:
        ...
        break;
    case Report::ChargesByDrivers:
        model->setHorizontalHeaderLabels({"id", "ID Водителя", "KWH", "Время"});
        for (const QVariant &charge : ReportOperations::getChargesDriverReport(this->fromDate, this->toDate))
        {
            QVariantList charges = charge.toList();
            QList<QStandardItem *> row;
...
void GeneralReport::setBottomTable()
{
    ...
    case Report::ChargesByDrivers:
        for (const QVariant &charge : ReportOperations::getAllChargesDriverReport(this->fromDate, this->toDate))
        {
            QVariantList charges = charge.toList();
            model->setHorizontalHeaderLabels({
                "Итого",
...
void GeneralReport::on_ToPDFButton_clicked()
{
    ...
    case Report::ChargesByDrivers:
        title = "Отчет по зарядкам по водителям";
        break;
...
void GeneralReport::on_PDFbyDate_clicked()
{
    QString title;
    int start = 1;
    switch (this->mode)
