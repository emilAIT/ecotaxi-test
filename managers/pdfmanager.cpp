...
// width: 20%; /* Увеличиваем ширину столбца с датой */
// }
// h1, h2, p, td {
//   text-align: center;
// }
...
//     QString title = "Daily_Report_" + QDate::currentDate().toString("dd.MM.yyyy");
...
//     // Получаем отфильтрованные данные после выбора пользователем колонок
//     QAbstractItemModel* filteredModel = model;

//     // Вывод структуры модели в терминал
//     qDebug() << "===== Структура модели =====";
//     qDebug() << "Количество строк: " << filteredModel->rowCount();
//     qDebug() << "Количество колонок: " << filteredModel->columnCount();
//     qDebug() << "Заголовки колонок:";

//     for (int j = 0; j < filteredModel->columnCount(); j++) {
//         qDebug() << "  [" << j << "] " << filteredModel->headerData(j, Qt::Horizontal).toString();
//     }

//     qDebug() << "Данные модели:";

//     for (int i = 0; i < filteredModel->rowCount(); i++) {
//         QStringList rowData;
//         for (int j = 0; j < filteredModel->columnCount(); j++) {
//             rowData << filteredModel->index(i, j).data(Qt::DisplayRole).toString();
//         }
//         qDebug() << "  [" << i << "] " << rowData.join(" | ");
//     }
//     qDebug() << "============================";

//     // Определяем, в каком столбце дата
...
//         }
//     }
//     qDebug() << "Дата найдена в столбце:" << dateColumnIndex;
...
//         if (!date.isValid()) {
//             date = dateVariant.toDate();  // Пробуем напрямую, если это QDate
//         }

//         if (!date.isValid()) {
//             qDebug() << "Некорректная дата в строке" << i << ":" << dateVariant.toString();
//             continue;
//         }
...
//     qDebug() << "Количество уникальных дат: " << groupedData.keys().size();
//     for (const QDate &date : groupedData.keys()) {
//         qDebug() << "Дата:" << date.toString("dd.MM.yyyy") << ", Количество строк:" << groupedData[date].size();
//     }

//     QList<QDate> sortedDates = groupedData.keys();
//     std::sort(sortedDates.begin(), sortedDates.end());

//     // Генерация HTML без пустой первой страницы
//     QString html;
//     bool firstPage = true;
//     for (const QDate& date : sortedDates) {
//         if (groupedData[date].isEmpty()) {
//             qDebug() << "Пропускаем пустую дату:" << date.toString("dd.MM.yyyy");
//             continue;
//         }

//         if (!firstPage) {
//             html += "<div style='page-break-before: always'></div>";  // Разрыв страницы только после первой даты
//         }
//         firstPage = false;

//         html += "<h2>Отчет за " + date.toString("dd.MM.yyyy") + "</h2>";
//         html += modelToHTML(groupedData[date], headers); // Теперь передаем оба аргумента
//     }

//     qDebug() << "HTML перед генерацией PDF:" << html;

//     // Вызываем `createPDF()`, передавая готовый HTML
...
//         html += "<h2>Отчет за " + date.toString("dd.MM.yyyy") + "</h2>";
//         html += modelToHT(model); // Теперь передаем оба аргумента
//     }
...
// QString PDFmanager::modelToHT(QAbstractItemModel *model)
// {
//     if (!model)
...
//     {
//         QString header = model->headerData(j, Qt::Horizontal).toString().toLower();
//         if (header.contains("дата") || header.contains("date"))
//         {
//             dateColumnIndex = j;
//             break;
//         }
//     }
//     if (dateColumnIndex == -1) return ""; // Если колонка с датой не найдена

//     // Сортируем модель по дате
//     QMap<QDate, QList<QList<QVariant>>> groupedData;
//     for (int i = 0; i < model->rowCount(); i++)
//     {
//         QVariant dateVariant = model->index(i, dateColumnIndex).data(Qt::DisplayRole);
//         QDate date = QDate::fromString(dateVariant.toString(), "yyyy-MM-dd");
...
//         QList<QVariant> row;
//         for (int j = 0; j < model->columnCount(); j++) {
//             row.append(model->index(i, j).data(Qt::DisplayRole));
//         }
//         groupedData[date].append(row);
//     }
...
//     // Генерация HTML с разбиением по датам
//     for (const QDate& date : sortedDates) {
//         if (!firstPage) {
...
//         // Заполняем таблицу данными за конкретный день
//         for (const QList<QVariant>& row : groupedData[date]) {
//             html += "<tr>";
...
//             if (header == "Дата")
//             {
//                 cellStyle += " white-space: nowrap; font-size:5px;";
//             }

//             // Применяем цвет к колонке "Инвестору"
//             if (header == "Инвестору" && start != 1)
//             {
//                 cellStyle += " color: #007700;";
//             }
