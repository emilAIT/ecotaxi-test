...
QVariantList ReportOperations::getCarsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    FLOOR((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) AS profit,\n"
        ...
        "        FLOOR(((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / events.nonZeroDays)\n"
        "    ELSE 0 END AS averageProfitPerDay,\n"
        "    COALESCE(cars.percentage, 0) AS percentage,\n"
        "    CASE WHEN ((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) >= 0 THEN\n"
        "        FLOOR(COALESCE(cars.percentage, 0) * ((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / 100)\n"
        "    ELSE 0 END AS ourIncome,\n"
        "    CASE WHEN ((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) >= 0 THEN\n"
        "        FLOOR((100 - COALESCE(cars.percentage, 0)) * ((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / 100)\n"
        "    ELSE FLOOR((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome)\n"
...
QVariantList ReportOperations::getAllCarsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "                FLOOR(cars.percentage * ((events.income * 0.90) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / 100)\n"
        "            ELSE \n"
        "                0 \n"
        "        END AS ourIncome,\n"
        "        CASE \n"
...
QVariantList ReportOperations::getInvestorsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    FLOOR(SUM(\n"
        "        (COALESCE(event_stats.income, 0) * 0.90 - COALESCE(charge_stats.totalKwh, 0) + COALESCE(event_stats.outcome, 0))\n"
        "    )) AS totalProfit,\n"
        "    FLOOR(SUM(\n"
        "        CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.90 - COALESCE(charge_stats.totalKwh, 0) + COALESCE(event_stats.outcome, 0)) >= 0 THEN\n"
        "                (COALESCE(event_stats.income, 0) * 0.90 - COALESCE(charge_stats.totalKwh, 0) + COALESCE(event_stats.outcome, 0)) \n"
        ...
        "            ELSE (COALESCE(event_stats.income, 0) * 0.90 - COALESCE(charge_stats.totalKwh, 0) + COALESCE(event_stats.outcome, 0))\n"
        "        END\n"
        "    )) AS investorsIncome\n"
        "FROM investors\n"
...
QVariantList ReportOperations::getChargesByDriversReport(QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
QVariantList ReportOperations::getAllChargesByDriversReport(QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
QVariantList ReportOperations::getAllCarReport(int carId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        COALESCE(SUM(CASE WHEN amount > 0 THEN amount * 0.90 ELSE 0 END), 0) AS totalEventProfit\n"
        "    FROM events\n"
        "    WHERE carId = " +
        QString::number(carId) + "\n"
                                 "        AND date BETWEEN '" +
...
QVariantList ReportOperations::getInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    kwh,\n"
        "    outcome,\n"
        "    FLOOR(income * 0.90 - kwh + outcome) as profit,\n"
        "    percentage,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.90 - kwh + outcome) < 0) THEN \n"
        "            0 \n"
        "        ELSE \n"
        "            FLOOR((income * 0.90 - kwh + outcome) * COALESCE(percentage, 0) / 100) \n"
        "    END AS ourProfit,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.90 - kwh + outcome) < 0) THEN \n"
        "            FLOOR(income * 0.90 - kwh + outcome) \n"
        "        ELSE \n"
        "            FLOOR((income * 0.90 - kwh + outcome) * COALESCE(100 - percentage, 0) / 100) \n"
        "    END AS investorsProfit\n"
        "FROM car_stats;";
...
QVariantList ReportOperations::getAllInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        COALESCE(charge_stats.kwh, 0) AS kwh,\n"
        "        COALESCE(cars.percentage, 0) AS percentage,\n"
        "        FLOOR(COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) AS profit,\n"
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) < 0 THEN 0 \n"
        "            ELSE (COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) * (COALESCE(cars.percentage, 0) / 100) \n"
        "        END) AS ourProfit,\n"
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) < 0 THEN \n"
        "                (COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) \n"
        "            ELSE \n"
        "                (COALESCE(event_stats.income, 0) * 0.90 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) * ((100 - COALESCE(cars.percentage, 0)) / 100) \n"
        "        END) AS investorsProfit\n"
        "    FROM investors\n"
        "    LEFT JOIN cars ON cars.investorId = investors.id\n"
        "    LEFT JOIN event_stats ON cars.id = event_stats.carId\n"
...
//мои изменения2
QVariantList ReportOperations::getChargesByDriverReport(int driverId, QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
// {
//     dbManager &db = dbManager::getInstance();
//     QString query = "SELECT charges.date, drivers.name, cars.sid, charges.kwh, charges.duration "
//                     "FROM charges "
//                     "LEFT JOIN drivers ON drivers.id = charges.driverId "
//                     "LEFT JOIN cars ON cars.id = charges.carId "
...
//мои изменения2
QVariantList ReportOperations::getAllChargesByDriverReport(int driverId, QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
// {
//     dbManager &db = dbManager::getInstance();
//     QString query = "SELECT SUM(charges.kwh) "
//                     "FROM charges "
//                     "WHERE charges.driverId = " + QString::number(driverId) +
//                     " AND charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") + "'";
//     return db.executeGet(query);
