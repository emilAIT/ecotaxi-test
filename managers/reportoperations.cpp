...
QVariantList ReportOperations::getCarsReportByDays(QDate fromDate, QDate toDate)
{
    ...
    for (const QDate &date : reportsByDay.keys()) {
        ...
        if (!date.isValid())
            dayReport["date"] = "Дата отсутствует";
        else
...
QVariantList ReportOperations::getAllDriversChargesReport( QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
QVariantList ReportOperations::getAllDriversChargesByCarReport(int driverId, QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
    ...
    QString query =
        "WITH ChargesByCar AS ( "
        "    SELECT charges.carId AS carId, "
        "           SUM(COALESCE(charges.kwh, 0)) AS kwhSum, "
        "           SUM(COALESCE(charges.duration, 0)) AS durationSum "
        "    FROM charges "
        "    WHERE charges.driverId = " + QString::number(driverId) + " "
                                      "      AND charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") + "' "
                                                                                        "    GROUP BY charges.carId "
                                                                                        ") "
                                                                                        "SELECT 0 AS dummy, '' AS dummyText, COALESCE(SUM(kwhSum), 0) AS totalKwh, COALESCE(SUM(durationSum), 0) AS totalDuration "
                                                                                        "FROM ChargesByCar;";
    ...
    {
        // Если нет данных, создаём строку с 4 столбцами, чтобы избежать index out of range
        QVariantList dummy;
        dummy.append(0);    // dummy
        dummy.append("");   // dummyText
        dummy.append(0);    // totalKwh
        dummy.append(0);    // totalDuration
        result.append(dummy);
    }
    return result;
}

QVariantList ReportOperations::getDriversChargesByCarReport(int id, QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
    QString query =
        ...
        "WHERE charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") + "'\n"
                                                                                        "  AND drivers.id = " + QString::number(id) + "\n"
