...
QVariantList ReportOperations::getCharges2Report(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    WHERE charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") + "'\n"
                                                                                        ...
                                                                                        ")\n"
                                                                                        "SELECT\n"
                                                                                        "    ROW_NUMBER() OVER (ORDER BY driverName) AS rowIndex,\n"
                                                                                        "    driverName,\n"
                                                                                        "    totalKWH\n"  // Выводим общую сумму KWH
                                                                                        "FROM DriverCharges";
    // Генерация индекса строки и выбор данных из временной таблицы
    ...
    for (const QVariant &item : result) {
        ...
        if (row.size() > 1) {
            QString driverName = row[0].toString();  // Имя водителя
            int chargeCount = row[1].toInt();  // Количество зарядок
            QString asd = row[2].toString();
            qDebug() << driverName << chargeCount << asd ;  // Выводим имя водителя и количество зарядок
        }
    }
