...
// В файле reportoperations.cpp
QVariantList ReportOperations::getDriverChargeReport(int driverId, QDate fromDate, QDate toDate)
{
    dbManager &db = dbManager::getInstance();
    QVariantList data = db.executeGet("SELECT charges.kwh, charges.date "
                                      "FROM charges "
                                      "WHERE charges.driverId = " + QString::number(driverId) +
                                      " AND charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") +
