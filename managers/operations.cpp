...
QVariantList Operations::selectAllDriveCharges(QDate fromDate, QDate toDate)
{
    dbManager &db = dbManager::getInstance();
    QVariantList data = db.executeGet("SELECT drivers.name AS driverName, SUM(charges.kwh) AS totalKwh "
                                      "FROM charges "
                                      "INNER JOIN drivers ON charges.driverId = drivers.id "
                                      "WHERE charges.date BETWEEN '" + fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") + "' "
                                                                                                                      "GROUP BY drivers.name");
    QVariantList driveCharges;
    foreach (QVariant row, data)
    {
        driveCharges.append(row);
    }
    return driveCharges;
}
