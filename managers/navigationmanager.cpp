...
bool navigationManager::openReport(int index, int id, QDate from, QDate to)
{
    ...
    case 15:
        ...
        break;
    case 16:
        this->GReportPage->setReport(Report::DriversCharge, from, to);
        lastFunc = [this](QDate from=QDate(), QDate to=QDate()) {this->GReportPage->setReport(Report::DriversCharge, from, to);};
        changeWindow(3);
        break;
    case 17:
        this->ReportsPage->setReport(Report::DriversCharge, id, from, to);
        lastFunc = [this](int id=-2, QDate from=QDate(), QDate to=QDate()) { this->ReportsPage->setReport(Report::DriversCharge, id, from, to); };
        changeWindow(4);
        break;
    default:
        return false;
